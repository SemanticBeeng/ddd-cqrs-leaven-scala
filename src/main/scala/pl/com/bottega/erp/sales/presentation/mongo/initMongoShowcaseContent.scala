...
object initMongoShowcaseContent extends (() => Unit) {
  ...
  private def init(db: MongoDB, presentationDB: MongoDB)
  {
    ...
    for (i <- 1 to 20)
    {
      val p1 = product(i*10 + 1, "Electronic Gizmo " + i, Standard,  new Money(0.99))
      collection += p1._1
      readCollection += p1._2

      val p2 = product(i*10 + 2, "Cell Phone with 32GB flash memory " + i, Standard,  new Money(299.99))
      collection += p2._1
      readCollection += p2._2

      val p3 = product(i*10 + 3, "Software Engineering Audiobook" + i, Food,  new Money(17.5))
      collection += p3._1
      readCollection += p3._2

      val p4 = product(i*10 + 4, "PC Game including Zombies Part " + i, Drug,  new Money(39.89))
      collection += p4._1
      readCollection += p4._2

      val p5 = product(i*10 + 5, "Tablet with keyboard" + i, Standard,  new Money(459.99))
      collection += p5._1
